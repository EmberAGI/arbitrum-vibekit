...
) => Promise<TArgs | Task | Message>;

/**
 * After hook function type that transforms the result into a Task/Message and handles transaction signing
 */
 ...
 */
export interface LiquidationPreventionHookConfig<TArgs, TResult, TContext = any, TSkillInput = any> {
  before?: BeforeHook<TArgs, TContext, TSkillInput>;
  after?: AfterHook<TResult, TArgs, TContext, TSkillInput>;
}

/**
 * Wraps a VibkitToolDefinition with before/after hooks for the liquidation prevention agent.
 * Before hooks can transform input arguments or return a Task/Message to short-circuit.
 * After hooks transform the raw result into a Task/Message and handle secure transaction signing.
 *
...
export function withHooks<TParams extends z.ZodTypeAny, TResult, TContext = any, TSkillInput = any>(
  ...
  hooks: LiquidationPreventionHookConfig<z.infer<TParams>, TResult, TContext, TSkillInput>
  ...
  return {
    ...
    execute: async (args, context) => {
      ...
      // Apply after hook if present (required for transaction signing)
      if (hooks.after) {
