...
export const jobManagementSkill = defineSkill({
  ...
  examples: [
    ...
    'CREATE SAFE WALLET JOB: Create a time-based job with Safe wallet mode. Extract ALL fields from jobDetails: jobTitle, scheduleType, timeInterval, timezone, safeAddress, dynamicArgumentsScriptUrl, language. Pass all to createTimeJobTool. NOTE: Safe wallet mode does NOT need targetContractAddress, targetFunction, or abi.',
    ...
    'CREATE INTERVAL JOB WITH SAFE: When jobDetails contains safeAddress, extract AND pass safeAddress, dynamicArgumentsScriptUrl, and language to createTimeJobTool. Required fields: jobTitle, scheduleType, timeInterval, timezone, safeAddress, dynamicArgumentsScriptUrl. Optional: chainId, timeFrame, walletMode, language (e.g., "go", "javascript", "python").',
  ...
  handler: async (input: any) => {
    ...
    if (input.operation === 'create' && input.jobType === 'time' && input.jobDetails) {
      console.log('📋 Extracting ALL fields from jobDetails:', JSON.stringify(input.jobDetails, null, 2));
      ...
      const extracted: any = {
        ...
        language: input.jobDetails.language || '',
      ...
      console.log('🔗 Calling createTimeJobTool with extracted fields');
    ...
    console.log('🤖 Delegating to LLM orchestration for operation:', input.operation);
