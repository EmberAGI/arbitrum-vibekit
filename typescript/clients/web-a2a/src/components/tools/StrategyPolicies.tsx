  ...
  return (
    <Card className="bg-[#2a2a2a] border-[#323232] rounded-xl">
      <CardHeader className="pb-4 flex flex-row items-center justify-between">
        <CardTitle className="text-lg font-semibold text-white">Policies</CardTitle>
        <Button
          variant="ghost"
          size="sm"
      ...
      <CardContent className="space-y-2">
        {policies.map((policy, idx) => {
          ...
          return (
            <div key={idx} className="bg-[#1a1a1a] rounded-lg overflow-hidden">
              <div
                    ...
                    {!isExpanded && (
                      <div className="flex items-center gap-2 mt-1">
                        {policy.assets && policy.assets.length > 0 && (
                          <span className="text-xs text-gray-500">{policy.assets.join(', ')}</span>
                        )}
                        ...
                        {policy.amount && (
                          <>
                            <span className="text-gray-700">•</span>
                            <span className="text-xs text-gray-400">${policy.amount}</span>
                          </>
                        )}
                      </div>
                    )}
                  ...
                  <div className="space-y-3">
                    {policy.assets && policy.assets.length > 0 && (
                      <div>
                        ...
                        <p className="text-sm text-white">{policy.assets.join(', ')}</p>
                      </div>
                    )}
                    {policy.protocols && policy.protocols.length > 0 && (
                      <div>
                        <p className="text-xs text-gray-500 mb-1">Protocols</p>
                        <p className="text-sm text-white">{policy.protocols.join(', ')}</p>
                      </div>
                    )}
                    {policy.amount && (
                      <div>
