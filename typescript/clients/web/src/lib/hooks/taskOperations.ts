...
export interface TaskOperations {
  addTask: (sessionId: string, taskId: string, taskState?: TaskState) => void;
...
export function createTaskOperations(
  ...
  const addTask = (sessionId: string, taskId: string, taskState: TaskState = 'pending') => {
    setState((prev) => {
  ...
  const updateTaskState = (
    ...
    setState((prev) => {
      ...
      if (taskIndex === -1) {
        console.warn('[TaskOperations] Task not found in session', sessionId, ':', taskId);
        return prev;
      }
