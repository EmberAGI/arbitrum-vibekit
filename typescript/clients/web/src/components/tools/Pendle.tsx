    ...
    <>
      {markets && isMarketList && (
        <div className="flex flex-col gap-2 p-8 bg-transparent shadow-md rounded-2xl text-white border-red-200 border-2">
          <h2 className="text-lg font-semibold mb-4">Available Pendle Markets:</h2>
          <Accordion type="single" collapsible>
            {markets?.map((market) => (
              ...
              >
                <AccordionTrigger>
                  {market.data?.name} ({market.data?.expiry?.split('T')[0]}) Aggr. APY:{' '}
                  ...
                  >
                    ...
                    <p className="font-normal w-full bg-zinc-600 rounded-full p-2 px-4 mb-4">
                      <span className="font-normal  text-sm flex gap-2">
                        <ShortAddress
                          web3Address={market.data?.underlyingAsset?.tokenUid?.address}
                        />{' '}
                        {' on chain '} {market.data?.underlyingAsset?.tokenUid?.chainId}
                      </span>
                    </p>
                  </div>
                </AccordionContent>
      ...
      {txPlan && txPreview && (
        <div className="flex flex-col gap-2 p-8 bg-transparent shadow-md rounded-2xl text-white border-red-200 border-2">
          ...
          <div className="rounded-xl bg-zinc-700 p-4 flex flex-col gap-2">
            <span className="font-normal flex gap-3 w-full items-center text-sm">From: </span>
            ...
            <p className="font-normal w-full bg-zinc-600 rounded-full p-2">
              <span className="font-normal  text-sm">{txPreview?.fromTokenAddress} </span>
            </p>

            <div className="border-t border-gray-500 my-2" />
            <span className="font-normal flex gap-3 w-full items-center text-sm">To: </span>
            ...
            <p className="font-normal w-full bg-zinc-600 rounded-full p-2">
              <span className="font-normal  text-sm">{txPreview?.toTokenAddress} </span>
            </p>
          </div>
