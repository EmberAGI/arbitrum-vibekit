            ...
            {positions ? (
                ...
                <div className="rounded-lg bg-black/20 p-4 space-y-3">
                    <div className="flex items-center gap-2">
                        ...
                        <h2 className="text-sm font-semibold text-orange-400">Liquidity Positions ({positions.length})</h2>
                    </div>
                    {positions.map((x) => (
                        <div key={x.tokenId + x.poolAddress} className="bg-black/30 rounded p-3 space-y-2">
                            <div className="flex items-center justify-between">
                                <span className="text-sm font-semibold text-white">{x.symbol0} / {x.symbol1}</span>
                                <span className="text-xs text-gray-500">#{x.tokenId}</span>
                            </div>
                            <div className="grid grid-cols-2 gap-2">
                                <div>
                                    <div className="text-xs text-gray-500">Amount 0</div>
                                    <div className="text-sm font-semibold text-white">{x.amount0} {x.symbol0}</div>
                                </div>
                                <div>
                                    <div className="text-xs text-gray-500">Amount 1</div>
                                    <div className="text-sm font-semibold text-white">{x.amount1} {x.symbol1}</div>
                                </div>
                            </div>
                            <div className="text-xs text-gray-600">
                                Range: {strToDecimal(x.positionRange.fromPrice)} → {strToDecimal(x.positionRange.toPrice)}
                            </div>
                ...
                <div className="rounded-lg bg-black/20 p-4 space-y-3">
                    <div className="flex items-center gap-2">
                        <Droplets className="h-4 w-4 text-orange-400" />
                        <h2 className="text-sm font-semibold text-orange-400">Available Pools ({pools.length})</h2>
                    </div>
                    {pools.map((x) => (
                        <div key={x.handle + x.price} className="bg-black/30 rounded p-3 space-y-2">
                            <div className="text-sm font-semibold text-white">{x.symbol0} / {x.symbol1}</div>
                ...
                txPreview?.action && (
                    <div className="rounded-lg bg-black/20 p-4 space-y-3">
                        <div className="flex items-center gap-2">
                            <Droplets className="h-4 w-4 text-orange-400" />
                            <h2 className="text-sm font-semibold text-orange-400">{txPreview?.action?.toUpperCase()}</h2>
                        </div>
                        ...
                        <div className="bg-black/30 rounded p-3 space-y-2">
                            ...
                            <div className="grid grid-cols-2 gap-2">
                                <div>
                                    ...
                                    <div className="text-sm text-orange-400">{txPreview?.token0Symbol?.toUpperCase()}</div>
                                </div>
                                <div>
                                    <div className="text-xs text-gray-500">Token 1</div>
                                    <div className="text-lg font-bold text-white">{txPreview?.token1Amount}</div>
                                    <div className="text-sm text-orange-400">{txPreview?.token1Symbol?.toUpperCase()}</div>
                                </div>
                            </div>
