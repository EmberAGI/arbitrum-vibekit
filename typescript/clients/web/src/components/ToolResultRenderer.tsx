...
export function ToolResultRenderer({
  ...
  const isInteractiveComponent =
    toolConfig?.category === 'interactive' && componentName !== 'JsonViewer';
    ...
    <Card className="w-full component-fade-in">
        ...
        <div className="flex items-start justify-between">
          <div className="flex-1">
            ...
            {toolConfig?.description && (
              <CardDescription className="mt-1">{toolConfig.description}</CardDescription>
            )}
          </div>
      ...
      <CardContent>
        {viewMode === 'json' || componentName === 'JsonViewer' ? (
          <JsonViewer data={result} title={`${toolConfig?.name || toolName} Result`} />
        ) : (
          <Suspense
            fallback={
              <div className="flex items-center justify-center py-8">
