...
function mcpProxy({
  ...
  transportToClient.onmessage = (message) => {
    ...
    transportToServer.send(message).catch((error) => {
      ...
      if ('id' in message && message.id !== undefined && !transportToClientClosed) {
        const errorResponse = {
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const transportType = searchParams.get('transportType') || 'streamable-http';
    const sessionId = request.headers.get('mcp-session-id');
    ...
    if (sessionId) {
      ...
      if (!transport) {
        ...
        console.log(`[MCP API] Session ${sessionId} not found, creating new session...`);

        if (!url) {
          return NextResponse.json({ error: 'Missing URL for new session' }, { status: 400 });
        }
        ...
        try {
          ...
          console.log('[MCP API] Created server transport for session recreation');
        } catch (error: any) {
        ...
        console.log('[MCP API] Started web app transport for session recreation');
        ...
        return new Promise((resolve) => {
          ...
          webAppTransport.handleRequest(mockReq, mockRes, parsedBody).catch((error: any) => {
      ...
      if (parsedBody && parsedBody.method === 'initialize') {
        console.log('[MCP API] Forwarding initialize request for existing session');
      }
      ...
      return new Promise((resolve) => {
        ...
        transport.handleRequest(mockReq, mockRes, parsedBody).catch((error: any) => {
          console.error('[MCP API] Request handling error:', error);
          resolve(
            NextResponse.json(
              { error: 'Failed to handle request', details: error.message },
