...
interface CoinMarqueeWidgetProps {
...
export function CoinMarqueeWidget({ 
  selectedCoins, 
  selectedCurrency, 
  appearance, 
  isScriptLoaded 
}: CoinMarqueeWidgetProps) {
  if (!isScriptLoaded) {
    return (
      <div className="flex items-center justify-center h-full border rounded-lg bg-muted/20">
        <div className="text-center text-muted-foreground">
          ...
          <div className="text-sm">Loading marquee...</div>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="border rounded-lg p-4 bg-muted/50 h-full">
      <h4 className="text-sm font-medium mb-2">
        {coinNames} Price Marquee ({selectedCurrency.toUpperCase()})
      </h4>
      <div className="w-full">
        <gecko-coin-price-marquee-widget 
          ...
          key={`marquee-${coinIds}-${selectedCurrency}-${appearance.lightMode}-${appearance.transparentBackground}-${appearance.outlined}`}
        />
      </div>
    </div>
  );
