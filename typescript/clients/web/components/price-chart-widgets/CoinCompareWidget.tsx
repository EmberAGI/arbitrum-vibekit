...
export function CoinCompareWidget({ 
  selectedCoins, 
  selectedCurrency, 
  appearance, 
  isScriptLoaded 
}: CoinCompareWidgetProps) {
  if (!isScriptLoaded) {
    return (
      <div className="flex items-center justify-center h-full border rounded-lg bg-muted/20">
        <div className="text-center text-muted-foreground">
          ...
          <div className="text-sm">Loading comparison chart...</div>
        </div>
      </div>
    );
  }
  ...
  const coinNames = selectedCoins.length > 1 
    ? `${selectedCoins.length} Coins Comparison`
    : selectedCoins[0]?.name + ' Comparison' || 'No Coins';

  return (
    <div className="border rounded-lg p-4 bg-muted/50 h-full">
      <h4 className="text-sm font-medium mb-2">
        {coinNames} ({selectedCurrency.toUpperCase()})
      </h4>
      <div className="w-full">
        <gecko-coin-compare-chart-widget 
          ...
          key={`compare-${coinIds}-${selectedCurrency}-${appearance.lightMode}-${appearance.transparentBackground}-${appearance.outlined}`}
        />
      </div>
    </div>
  );
