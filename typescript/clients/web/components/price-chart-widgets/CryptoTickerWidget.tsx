...
export function CryptoTickerWidget({ 
  selectedCoin, 
  selectedCurrency, 
  appearance, 
  isScriptLoaded 
    ...
    return (
      <div className="flex items-center justify-center h-full border rounded-lg bg-muted/20">
        <div className="text-center text-muted-foreground">
          ...
          <div className="text-sm">Loading ticker...</div>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="border rounded-lg p-4 bg-muted/50 h-full">
      <h4 className="text-sm font-medium mb-2">
        {selectedCoin.name} Ticker ({selectedCurrency.toUpperCase()})
      </h4>
      <div className="w-full">
        <gecko-coin-ticker-widget 
          ...
          key={`ticker-${selectedCoin.id}-${selectedCurrency}-${appearance.lightMode}-${appearance.transparentBackground}-${appearance.outlined}`}
        />
      </div>
    </div>
  );
