  ...
  return (
    <div className="border rounded-lg p-4 bg-muted/50 h-full">
      <h4 className="text-sm font-medium mb-2">
        {selectedCoin.name} ({selectedCurrency.toUpperCase()})
      </h4>
      <div className="w-full">
        <gecko-coin-price-chart-widget 
          ...
          key={`price-chart-${selectedCoin.id}-${selectedCurrency}-${appearance.lightMode}-${appearance.transparentBackground}-${appearance.outlined}`}
        />
      </div>
    </div>
  );
