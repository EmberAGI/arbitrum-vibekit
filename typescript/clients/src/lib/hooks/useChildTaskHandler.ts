  ...
  const handleChildTask = useCallback(
    (
      ...
      if (processedChildTasksRef.current.has(childTaskId)) {
        console.log(
          "[ChildTaskHandler] Child task already processed, skipping:",
          childTaskId,
        );
        return;
      }
      ...
      const childSession = sessions[childSessionId] || {
        id: childSessionId,
      ...
      reconnectToStream(childSession, {
        onMessage: (sessionId, messageId, content, sender, updates) => {
          ...
          if (justSubmittedDelegations && messageId && (!updates?.artifacts || (!updates.artifacts["delegations-display"] && !updates.artifacts["delegations-data"])) && !hasDashboardArtifacts) {
            console.log("[ChildTaskHandler] BLOCKING update - preserving delegation artifacts after submission");
            return messageId;
          }
        ...
        onStatusUpdate: (sessionId, status, data) => {
          ...
          if (data?.awaitingInput) {
            if (lastMessageIdRef.current) {
              updateMessageInSession(sessionId, lastMessageIdRef.current, {
