...
export function useTransactionExecutor({
  ...
  const processTx = useCallback(
    async (transaction: RawTransaction | undefined, isApproval: boolean) => {
      ...
      console.log(
        `[processTx] Start ${
          isApproval ? `Approval #${approvalIndex + 1}` : "Main Tx"
      ...
      try {
        ...
        console.log(
          `[processTx] sendTransactionAsync finished for ${
            isApproval ? "approval" : "main"
          } tx.`
        );
        ...
        if (isApproval) {
          ...
          console.log(
            `[processTx] Approval ${
              approvalIndex + 1
            }/${totalApprovals} submitted successfully. Advanced index.`
          );
        }
      } catch (err: any) {
        console.error(
          `[processTx] Error during ${
            isApproval ? "approval" : "main"
          } tx processing:`,
          err
        );
  ...
  useEffect(() => {
    ...
    if (readyForNextApproval) {
      ...
      if (!isProcessingTx) {
        ...
        console.log(
          `[AutoApprove Effect] Calling approveNext for index ${
            approvalIndex + 1
          }`
        );
