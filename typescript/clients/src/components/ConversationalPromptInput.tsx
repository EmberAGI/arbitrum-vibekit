...
import { findPromptMapping, getPromptMappingsByCategory } from '@/config/promptMappings';
import { useCompletionState } from '@/lib/hooks/useCompletionState';

interface ConversationalPromptInputProps {
    onSubmit: (prompt: string, template?: PromptTemplate, parameters?: Record<string, string>) => void;
    placeholder?: string;
    className?: string;
    handleCompletion?: (
...
const ConversationalPromptInput = React.forwardRef<HTMLInputElement, ConversationalPromptInputProps>(({
    ...
    const promptTemplates = React.useMemo(() => {
        console.log("[ConversationalPromptInput] Converting MCP prompts to templates:", mcpPrompts.map(p => ({ name: p.name, description: p.description })));

        // If no MCP prompts, use static prompts as fallback
        if (!mcpPrompts || mcpPrompts.length === 0) {
            console.log("[ConversationalPromptInput] No MCP prompts available, using static prompts fallback");
        ...
        return mcpPrompts.map((mcpPrompt): PromptTemplate => {
            ...
            console.log(`[ConversationalPromptInput] Looking for mapping for "${mcpPrompt.name}":`, mapping ? "FOUND" : "NOT FOUND");
            ...
            if (mapping) {
                ...
                console.log(`[ConversationalPromptInput] Using mapping template for "${mcpPrompt.name}":`, mapping.template);
                return {
                    id: mcpPrompt.name,
                    name: mapping.name,
                ...
                console.log(`[ConversationalPromptInput] Using fallback template for "${mcpPrompt.name}":`, template);
                return {
                    id: mcpPrompt.name,
                    name: mcpPrompt.name,
    ...
    const generateFinalPrompt = (): string => {
        ...
        console.log('[generateFinalPrompt] Template parameters:', selectedTemplate.parameters.map(p => p.name));
    ...
    const handleSubmit = async () => {
        ...
        if (selectedTemplate && onGetPrompt && isConnected) {
            try {
                ...
                console.log("[ConversationalPromptInput] MCP prompt validated, using template-generated text:", finalPrompt);
        ...
        onSubmit(finalPrompt, selectedTemplate || undefined, selectedTemplate ? promptArgs : undefined);
                    ...
                    <div className="absolute bottom-full left-0 mb-2 min-w-[350px] z-50 bg-black/90 backdrop-blur-sm rounded-lg shadow-2xl max-h-80 overflow-hidden">
                        <div className="flex items-center gap-2 px-3 py-2 bg-orange-500/10">
                            ...
                            <span className="text-xs font-medium text-gray-300">
                                {filteredCompletions.length} suggestion{filteredCompletions.length !== 1 ? 's' : ''}
                            </span>
                        </div>
                        <Input
                            placeholder="Filter..."
                        ...
                        <div className="max-h-56 overflow-y-auto px-2 pb-2">
                            {filteredCompletions.length > 0 ? (
                                filteredCompletions.map((completion, idx) => (
                                    <div
                                        key={idx}
                                        className={`px-3 py-2 rounded cursor-pointer text-sm text-white transition-all ${idx === selectedIdx
                                            ? 'bg-orange-500/20'
                                            : 'hover:bg-white/5'
                                            }`}
                                        onClick={() => {
                                        ...
                                        onMouseEnter={() => setSelectedCompletionIndex(prev => ({ ...prev, [param.name]: idx }))}
                                    >
                                        {completion}
                                    </div>
                                ))
                ...
                <div className="flex flex-wrap items-center gap-2">
                    {parts.map((part, index) => {
                        ...
                        if (paramMatch) {
                            ...
                            if (param) {
                                const paramIndex = selectedTemplate.parameters.findIndex(p => p.name === paramName);
                                return (
                ...
                <div className="relative">
                    ...
                    {isPromptDropdownOpen && promptTemplates.length > 0 && (
                        ...
                        >
                            ...
                            {/* Categorized Prompts */}
                            <div className="max-h-[450px] overflow-y-auto pr-2" style={{
                                scrollbarWidth: 'thin',
