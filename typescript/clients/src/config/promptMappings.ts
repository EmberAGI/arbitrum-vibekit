...
export const promptMappings: PromptMapping[] = [
  {
    ...
    category: "Trading",
    example:
      "swap 100 USDC to ETH from ethereum to arbitrum with exactIn using wallet 0x123...",
    parameterOverrides: [
      {
        name: "amount",
        type: "number",
      ...
      },
      {
        ...
        description:
          "exactIn = exact amount to spend, exactOut = exact amount to receive",
      },
      {
        name: "walletAddress",
        type: "text",
  ...
  },
  {
    ...
    template:
      "Lend {token} on {protocol} on {chain} with wallet {walletAddress}",
    category: "Lending",
  ...
  },
  {
    ...
    category: "Lending",
    example:
      "borrow USDC using ETH as collateral on Aave on ethereum with wallet 0x123...",
  },
  {
    ...
    category: "Liquidity",
    example:
      "liquidity ETH and USDC on Uniswap on ethereum with wallet 0x123...",
  },
  {
    ...
    category: "Liquidity",
    example:
      "remove liquidity from ETH/USDC pool on Uniswap on ethereum with wallet 0x123...",
  },
  {
    mcpPromptId: "stake",
    name: "Stake Tokens",
    ...
    triggerWords: ["stake"],
    template:
      "Stake {token} on {protocol} on {chain} with wallet {walletAddress}",
    category: "Staking",
  ...
  },
  {
    ...
    template:
      "Unstake {token} from {protocol} on {chain} with wallet {walletAddress}",
    category: "Staking",
  ...
  },
  {
    ...
    template:
      "Bridge {amount} {token} from {fromChain} to {toChain} using wallet {walletAddress}",
    category: "Bridge",
 ...
 */
export function findPromptMapping(
  mcpPromptId: string
): PromptMapping | undefined {
  console.log(`[findPromptMapping] Looking for mapping for: "${mcpPromptId}"`);

  // First try exact match
  let mapping = promptMappings.find(
    (mapping) => mapping.mcpPromptId === mcpPromptId
  );
  if (mapping) {
...
export function searchPromptMappings(query: string): PromptMapping[] {
  ...
  return promptMappings.filter(
    (mapping) =>
      ...
      mapping.triggerWords.some((word) =>
