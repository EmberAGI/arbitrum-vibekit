...
8. Scheduled-run prompts execute in ephemeral runtime context; the durable contract is `AutomationRun` + `PiExecution` + root activity/projection, not a persisted user-visible `PiThread`.
...
sequenceDiagram
  ...
  RT-->>AG: root activity projection and run details available
  AG-->>Web: projected AG-UI state on attach/connect
  Web->>Web: show activity history and run details without transcript pollution
```
