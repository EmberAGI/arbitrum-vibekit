...
- Canceling automation must suspend future cadence and stop an active same-process scheduled invocation through the runtime stop path so later completion cannot advance cadence. The linked `PiExecution` must move to an unsuccessful terminal status when the active run is canceled, preserving operator inspection consistency.
...
- Rescheduled `AutomationRun.scheduled_at` values must use the future cadence timestamp derived from the terminal decision, matching `PiAutomation.next_run_at`, not the prior completion/tick-start timestamp.
...
- Previous-run scheduled context must prefer the actual persisted scheduled-run snapshot summary and artifact reference before falling back to generic root activity status text.
...
- Web-facing run inspection is a projection of that runtime-owned activity/artifact state. The web may render run ids, statuses, summaries, and artifact references, and must provide control-plane-backed inspect/open affordances for persisted run snapshots/artifacts. It must not infer automation truth from chat transcript messages, static labels, or local-only anchors.
