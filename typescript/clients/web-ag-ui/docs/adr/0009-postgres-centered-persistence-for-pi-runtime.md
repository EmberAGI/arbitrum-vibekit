...
- Web-facing run inspection is a projection of that runtime-owned activity/artifact state. The web may render run ids, statuses, summaries, and artifact references, and must provide root-thread-scoped control-plane-backed inspect/open affordances for persisted run snapshots/artifacts. Runtime control reads must filter automation runs and artifacts by the requested root-thread scope before id selection; the web must not infer automation truth from chat transcript messages, static labels, local-only anchors, or unscoped runtime-wide lists.
