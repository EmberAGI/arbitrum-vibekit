...
- Canceling automation must suspend cadence and cancel the current scheduled or already-running run. If the current run is active in the same process, cancellation must call the runtime stop path so terminal completion cannot continue to advance cadence.
