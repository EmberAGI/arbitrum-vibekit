...
import { createEmberLendingAgUiHandler, EMBER_LENDING_AGENT_ID } from './agUiServer.js';
import { prepareEmberLendingServer } from './startup.js';

const { bootstrap, port, service } = await prepareEmberLendingServer();
...
server.listen(port, () => {
  console.log(
    [
      `agent-ember-lending listening on http://127.0.0.1:${port}`,
      bootstrap
