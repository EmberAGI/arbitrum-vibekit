...
describe('createEmberLendingOnchainActionsAnchoredPayloadResolver', () => {
  ...
  it('surfaces ReserveFrozen() as an actionable borrow-planning failure instead of an unknown estimate-gas revert', async () => {
    const fetchImpl = vi
      .fn<typeof fetch>()
      .mockResolvedValueOnce(
        new Response(
    ...
    await resolver.anchorCandidatePlanPayload({
      agentId: 'ember-lending',
      threadId: 'thread-reserve-frozen-001',
      transactionPlanId: 'txplan-ember-lending-reserve-frozen-001',
      walletAddress: '0x00000000000000000000000000000000000000b1',
      rootUserWalletAddress: '0x00000000000000000000000000000000000000a1',
      payloadBuilderOutput: {
        transaction_payload_ref: 'txpayload-ember-lending-reserve-frozen-001',
        required_control_path: 'lending.borrow',
        network: 'arbitrum',
      },
      compactPlanSummary: {
    ...
    await expect(
      resolver.resolvePreparedUnsignedTransaction({
        agentId: 'ember-lending',
        executionPreparationId: 'execprep-ember-lending-reserve-frozen-001',
        transactionPlanId: 'txplan-ember-lending-reserve-frozen-001',
        requestId: 'req-ember-lending-reserve-frozen-001',
        canonicalUnsignedPayloadRef: 'unsigned-txpayload-ember-lending-reserve-frozen-001',
        delegationArtifactRef: TEST_ACTIVE_DELEGATION_ARTIFACT_REF,
        rootDelegationArtifactRef: TEST_ROOT_DELEGATION_ARTIFACT_REF,
        plannedTransactionPayloadRef: 'txpayload-ember-lending-reserve-frozen-001',
        walletAddress: '0x00000000000000000000000000000000000000b1',
        network: 'arbitrum',
        requiredControlPath: 'lending.borrow',
      }),
    ).rejects.toThrow(
      'Aave rejected the requested borrow because the reserve is frozen. Create a new borrow plan for a different asset.',
    );
