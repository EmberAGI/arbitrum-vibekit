...
describe('buildExecutionPlanArtifact', () => {
  it('wraps execution plan data into an artifact', () => {
    ...
    expect(artifact.parts[0]).toEqual({ kind: 'text', text: artifact.description });
    expect(artifact.parts[1]).toMatchObject({ kind: 'data', data: plan });
  ...
  it('surfaces submission and final execution hashes distinctly', () => {
    const artifact = buildExecutionResultArtifact({
      action: 'long',
      ok: true,
      txHashes: [
      ...
      lastTxHash: '0x9999999999999999999999999999999999999999999999999999999999999999',
      submissionTxHash: '0x1111111111111111111111111111111111111111111111111111111111111111',
      finalTxHash: '0x9999999999999999999999999999999999999999999999999999999999999999',
    });
    ...
    expect(artifact.parts[1]?.data).toMatchObject({
      ...
      submissionTxHash: '0x1111111111111111111111111111111111111111111111111111111111111111',
      finalTxHash: '0x9999999999999999999999999999999999999999999999999999999999999999',
      userFacingTxHash: '0x9999999999999999999999999999999999999999999999999999999999999999',
    });
  });
