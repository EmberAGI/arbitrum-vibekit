...
import { PI_RUNTIME_AGENT_CONFIGS, PI_EXAMPLE_AGENT_ID } from '@/config/piRuntimeAgents';
...
export const PI_EXAMPLE_AGENT_NAME = PI_EXAMPLE_AGENT_ID;
...
export function buildCopilotRuntimeAgents(env: RuntimeEnv) {
  ...
  for (const config of PI_RUNTIME_AGENT_CONFIGS) {
    agents[config.agentId] = new PiRuntimeGatewayHttpAgent({
      agentId: config.agentId,
      runtimeUrl: env[config.runtimeUrlEnvVar] || config.defaultRuntimeUrl,
    });
  }
