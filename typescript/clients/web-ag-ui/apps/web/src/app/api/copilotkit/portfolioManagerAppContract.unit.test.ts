...
const portfolioManagerAppRoot = path.resolve(process.cwd(), '../agent-portfolio-manager');
const packageJsonPath = path.join(portfolioManagerAppRoot, 'package.json');
const serverEntryPath = path.join(portfolioManagerAppRoot, 'src/server.ts');
const startupEntryPath = path.join(portfolioManagerAppRoot, 'src/startup.ts');
const agUiServerEntryPath = path.join(portfolioManagerAppRoot, 'src/agUiServer.ts');

describe('portfolio manager app package contract', () => {
  it('exists as a thin web-ag-ui app that depends on the public agent-runtime facade', () => {
    ...
    expect(packageJson.name).toBe('agent-portfolio-manager');
