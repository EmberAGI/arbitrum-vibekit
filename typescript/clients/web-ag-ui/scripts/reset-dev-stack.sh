...
ports=(3000 3001 3002 3003 3004 3005 8123 8124 8125 8126 8127)
...
kill_langgraph() {
  local pids
  pids=$(ps -axo pid=,command= | awk -v p1="langgraph-api/dist/cli/entrypoint.mjs" -v p2="apps/(agent|agent-clmm|agent-pendle|agent-gmx-allora|agent-polymarket)" '$0 ~ p1 && $0 ~ p2 {print $1}')
  kill_pids "$pids"
}
...
cleanup_state() {
  rm -rf apps/agent-clmm/.langgraph_api apps/agent/.langgraph_api apps/agent-pendle/.langgraph_api apps/agent-gmx-allora/.langgraph_api apps/agent-polymarket/.langgraph_api apps/web/.next/dev/lock
}
