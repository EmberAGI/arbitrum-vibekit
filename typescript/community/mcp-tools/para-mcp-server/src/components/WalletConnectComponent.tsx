...
export interface WalletConnectComponentProps {
...
export default function WalletConnectComponent({
  ...
  const [authStep, setAuthStep] = useState<AuthStep>(
    connectedAddress ? "connected" : "select",
  );
  const [status, setStatus] = useState<"idle" | "loading" | "error">("idle");
  const [message, setMessage] = useState(
    connectedAddress && connectedName ? `Connected to ${connectedName}` : "",
  );
  const [walletAddress, setWalletAddress] = useState<string>(
    connectedAddress ?? "",
  );
  const [availableProviders, setAvailableProviders] = useState<
    Array<EthereumProviderInfo>
  >([]);

  useEffect(() => {
    const providers: Array<EthereumProviderInfo> = [];
  ...
  return (
    <div className="w-full max-w-md rounded-2xl bg-white p-8 shadow-2xl">
      <h1 className="mb-2 text-3xl font-bold text-gray-900">Wallet Connect</h1>
      <p className="mb-8 text-sm text-gray-600">Connect your Ethereum wallet</p>
          ...
          {availableProviders.length > 0 ? (
            ...
            <div className="rounded-lg bg-yellow-50 p-4 text-center">
              <p className="text-sm text-yellow-800">
                No wallet provider detected. Please install MetaMask, Rainbow, or
                another Web3 wallet.
              </p>
            </div>
          )}
