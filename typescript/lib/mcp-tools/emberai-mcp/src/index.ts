...
const borrowSchema = {
  ...
  amount: z.string().describe('The amount to borrow (human readable format expected by SDK).'),
...
const withdrawSchema = {
  ...
  tokenChainId: z.string().describe('The chain ID where the token contract resides.'),
  amount: z.string().describe('The amount to withdraw (human readable format).'),
...
const supplyLiquiditySchema = {
  ...
  token0ChainId: z.string().describe('The chain ID where the token0 contract resides.'),
  token1Address: z
    .string()
    .describe('The contract address of the second token in the pair (token1).'),
  token1ChainId: z.string().describe('The chain ID where the token1 contract resides.'),
  amount0: z.string().describe('The amount of token0 to supply (human-readable format).'),
  amount1: z.string().describe('The amount of token1 to supply (human-readable format).'),
  priceFrom: z
    .string()
  ...
  userAddress: z.string().describe('The wallet address supplying the liquidity.'),
};

const withdrawLiquiditySchema = {
  tokenId: z.string().describe('The NFT token ID representing the liquidity position to withdraw.'),
  providerId: z
    .string()
    .describe(
      "The ID of the liquidity provider protocol (e.g., 'uniswap_v3'). Usually obtained from the getUserLiquidityPositions tool."
    ),
  userAddress: z.string().describe('The wallet address withdrawing the liquidity.'),
};
...
const getUserLiquidityPositionsSchema = {
  userAddress: z.string().describe('The wallet address to fetch liquidity positions for.'),
};
...
const getMarketDataSchema = {
  tokenAddress: z.string().describe('The contract address of the token to get market data for.'),
  tokenChainId: z.string().describe('The chain ID where the token contract resides.'),
};
...
type GetUserLiquidityPositionsParams = z.infer<
  ReturnType<typeof z.object<typeof getUserLiquidityPositionsSchema>>
>;
...
server.tool(
  ...
  async (params: SupplyLiquidityParams) => {
    ...
    try {
      ...
      console.error(`SupplyLiquidity tool success. Transactions:`, response.transactions);
...
server.tool(
  ...
  async (params: WithdrawLiquidityParams) => {
    ...
    try {
      ...
      console.error(`WithdrawLiquidity tool success. Transactions:`, response.transactions);
...
server.tool(
  ...
  async (params: GetLiquidityPoolsParams) => {
    ...
    try {
      ...
      const response: GetLiquidityPoolsResponse = await emberClient.getLiquidityPools(undefined);
...
server.tool(
  ...
  async (params: GetUserLiquidityPositionsParams) => {
    console.error(`Executing getUserLiquidityPositions tool with params:`, params);
