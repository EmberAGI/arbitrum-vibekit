...
async function testMcpClient() {
  ...
  try {
    ...
    const transport = new StdioClientTransport({
      command: "node",
      args: [
        "/Users/tomdaniel/Documents/Ember_Cognition_Inc/Software/arbitrum-agentkit/typescript/mcp-tools/emberai-mcp/dist/index.js",
      ],
    });
    ...
    if (toolNames.length > 0) {
      ...
      if (toolNames.length > 0) {
        ...
        const firstTool = toolsResponse.tools.find(
          (tool) => tool.name === firstToolName
        );
      ...
      if (toolNames.includes("getCapabilities")) {
        ...
        try {
          ...
          console.log(
            "getCapabilities result with name parameter:",
            JSON.stringify(result, null, 2)
          );
        } catch (error) {
          console.error(
            "Error calling getCapabilities with name parameter:",
            error
          );
          console.error("Error details:", JSON.stringify(error, null, 2));
        }
        ...
        console.log(
          "\ngetCapabilities tool not found. Available tools:",
          toolNames
        );
      }
    } else {
