...
const basePricePredictionTool: VibkitToolDefinition<typeof GetPricePredictionParams, any, any, any> = {
  ...
  description:
    'Get price prediction for a specific token from Allora prediction markets. Returns the predicted price value.',
  ...
  execute: async (args: any, context) => {
    ...
    try {
      ...
      if (predictionValue === 'N/A') {
        return createErrorTask(
          'predict-price',
          new VibkitError('PredictionUnavailable', -32603, 'Prediction value is not available in the response.'),
        );
      }

      // Return only the raw prediction value for tool chaining
...
export const getPricePredictionTool = withHooks(basePricePredictionTool, {
  before: async (args, context) => {
    ...
    return topicDiscoveryHook(withToken, context);
  },
});
