# Allora AI Trading Agent

This directory contains the `allora-trading-agent`, a sophisticated AI agent built on the Arbitrum Vibekit framework. It acts as an **LLM Orchestrator**, capable of understanding natural language commands to form and execute trading strategies.

## Architecture: An LLM-Powered Orchestrator

This agent demonstrates a powerful multi-agent workflow where an orchestrator agent uses specialized "tool" agents to achieve a complex goal.

- **`allora-price-prediction-agent` (Tool Agent)**: Its sole responsibility is to provide price predictions as a service.
- **`ember-mcp-tool-server` (Tool Agent)**: Its sole responsibility is to execute on-chain trades as a service.
- **`allora-trading-agent` (Orchestrator Agent)**: This is the agent you interact with. It has no internal logic for trading or predicting. Instead, it uses an LLM to understand your commands, calls the tool agents to gather information and execute actions, and provides natural language responses.

When you ask, "Should I buy BTC?", the orchestrator calls the prediction agent, analyzes the result, and gives you an informed opinion. If you then say "Okay, buy 100 USDC worth," it will call the Ember trading agent to perform the swap.

## Getting Started: Running with Docker Compose

The easiest way to run the entire multi-agent system is with Docker and Docker Compose. This will build both agents and run them in a shared network environment with a single command.
...
You will need to create a `.env` file in **two** locations with your specific secrets.

**A. Price Prediction Agent Config:**

- **File**: `typescript/templates/allora-price-prediction-agent/.env`
- **Content**:
  ```
  # Required for the prediction agent's LLM and connection to the Allora network
  OPENROUTER_API_KEY=your_openrouter_key
  ALLORA_API_KEY=your_allora_key
  ```

**B. Trading Agent Config:**

- **File**: `typescript/examples/allora-trading-agent/.env`
- **Content**:
  ```
  # Required for the trading agent's LLM and connection to other services
  OPENROUTER_API_KEY=your_openrouter_key
  ALLORA_AGENT_URL=http://price-prediction-agent:3008/sse
  EMBER_ENDPOINT=grpc.api.emberai.xyz:50051
  ```
- **Important**: Notice the `ALLORA_AGENT_URL`. We use the service name `price-prediction-agent` from our `docker-compose.yml` file, which Docker automatically resolves to the correct container IP address.

### 2. Run the System

Once your `.env` files are in place, navigate to the `typescript` directory in your terminal and run the following command to use the agent-specific compose file:
...
1.  Build the Docker image for the `price-prediction-agent`.
2.  Build the Docker image for the `trading-agent`.
3.  Start both containers. `depends_on` ensures the prediction agent starts first.
4.  You will see the logs from both agents interleaved in your terminal.

### 3. Interact with the Agent

With both agents running, you can now have a conversation with your trading agent using the Vibekit web frontend.

1.  **Start the Frontend**: In a new terminal, from the `typescript/` directory, run:
    ```bash
    pnpm dev --filter @vibekit/web
    ```
2.  **Connect**: Open your browser to `http://localhost:3000`. In the connection input, use the **host URL** for your trading agent:
    `http://localhost:3009/sse`
3.  **Execute**: Click connect, select the "Allora AI Trader" skill. Instead of a form with many fields, you can now use the chat interface to have a conversation. Try asking:
    - "What is the price prediction for ETH?"
    - "Should I buy some Arbitrum?"
    - "Get the latest forecast for BTC and if it looks good, buy 0.01 of it for me using USDC."
